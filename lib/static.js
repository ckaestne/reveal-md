...
const { revealBasePath, revealPluginPath, highlightThemePath } = require('./constants');
...
const cp = (source, target) => {
  console.log(`❏ ${readablePath(source)} → ${readablePath(target)}`);
...
const copyAssetsAndWriteFile = async (sourceDir, file, targetDir) => {
  ...
  const base = relativeDir(file, '.').replace("\\","/");
...
module.exports = async () => {
  ...
  await Promise.all([cp(revealPluginPath, path.join(staticDir, 'rplugin'))])

  const staticDirs = typeof options.staticDirs === 'string' ? options.staticDirs.split(',') : options.staticDirs;
  await Promise.all(
    staticDirs.map(dir => cp(path.join(process.cwd(), dir), path.join(staticDir, getAssetsDir(), dir)))
  );
